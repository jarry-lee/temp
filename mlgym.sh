#!/bin/bash
echo "Extracting MLOps Source Code..."

cat << '__EOF_FILE__' > "app.py"
...
def get_lot_codes():
    if config.has_section('settings') and config.has_option('settings', 'lot_codes'):
        raw_codes = config.get('settings', 'lot_codes')
        return [c.strip() for c in raw_codes.split(',')]
    return ['aaa']  # ultimate fallback
...
def save_model_weight(model_name, lot_cd, uploaded_file):
    ensure_dirs()
    ...
    weight_dir_path = os.path.join(model_path, "weight", lot_cd)
...
def get_weights(model_name):
    ...
    weights = []
    # Iterate through lot_cd subdirectories (aaa, bbb, ccc, etc.)
    for lot_cd in os.listdir(weight_base_dir):
        lot_dir_path = os.path.join(weight_base_dir, lot_cd)
        if not os.path.isdir(lot_dir_path):
            continue
            
        for item in os.listdir(lot_dir_path):
            if item.endswith(('.pt', '.pth', '.h5', '.pb', '.pbtxt', '.meta')):
                file_path = os.path.join(lot_dir_path, item)
                stat = os.stat(file_path)
                size_mb = stat.st_size / (1024 * 1024)
                upload_time = datetime.datetime.fromtimestamp(stat.st_mtime).strftime('%Y-%m-%d %H:%M:%S')
                
                # Extract version
                match = re.search(r'_v(\d+)', item)
                version_str = f"v{match.group(1)}" if match else "v1"
    ...
    # Sort by version descending for better UI presentation
    weights = sorted(weights, key=lambda x: int(x['Version'][1:]), reverse=True)
    return weights
...
def delete_weight(model_name, lot_cd, weight_filename):
    weight_path = os.path.join(MODELS_DIR, model_name, "weight", lot_cd, weight_filename)
...
cat << '__EOF_FILE__' > "config.ini"
[paths]
models_dir = models
data_dir = data

[settings]
lot_codes = aaa, bbb, ccc
__EOF_FILE__

mkdir -p "src"
cat << '__EOF_FILE__' > "src/ende.py"
import os
import json
...
mkdir -p "src"
cat << '__EOF_FILE__' > "src/Training.py"
import streamlit as st
import time
import tempfile
import os
...
mkdir -p "src"
cat << '__EOF_FILE__' > "src/Training_Graph.py"
import streamlit as st
import json
...
mkdir -p "views"
cat << '__EOF_FILE__' > "views/Training.py"
import views.page_training as pt; pt.render_page()
__EOF_FILE__

mkdir -p "views"
cat << '__EOF_FILE__' > "views/Training_Graph.py"
import views.page_training_graph as ptg; ptg.render_page()
__EOF_FILE__

mkdir -p "views"
cat << '__EOF_FILE__' > "views/Data.py"
import streamlit as st
import os
import glob
...
mkdir -p "views"
cat << '__EOF_FILE__' > "views/Validation.py"
import streamlit as st
import time
from PIL import Image
from utils import get_models, get_weights, get_lot_codes
...
with col1:
    ...
    if not models:
        ...
        if selected_model:
            ...
            if not lot_weights:
                st.warning(f"No weights available for this model in Lot `{selected_lot_env}`.")
                selected_weight = None
            else:
                def format_weight(w):
                    return f"{w['Filename']} ({w['Version']})"
                
                selected_weight_obj = st.selectbox("Choose a weight file", lot_weights, format_func=format_weight)
                selected_weight = selected_weight_obj['Filename'] if selected_weight_obj else None
...
mkdir -p "views"
cat << '__EOF_FILE__' > "views/Models.py"
import streamlit as st
import os
...
mkdir -p "views"
cat << '__EOF_FILE__' > "views/Deployment.py"
import streamlit as st
...
mkdir -p "css"
cat << '__EOF_FILE__' > "css/animate.min.css"
@charset "UTF-8";/*!
...
chmod +x build.sh
echo "Extraction complete. You can now run ./build.sh to compile the modules."
