...
if __name__ == "__main__":
    ...
    input_shape = (1024, 1024, 3) # Using 512 for memory efficiency during pre-training if needed
    ...
    batch_size = 4 # Small batch size for large images
    data_gen = ContrastivejhDataGenerator(image_paths, batch_size=batch_size, input_shape=input_shape)
    ...
    epochs = 10 # 5 is likely too few for convergence, but good for verification
    history = contrastive_model.fit(data_gen, epochs=epochs)
    
    # Save Encoder
    print("\n4. Saving pre-trained encoder...")
    os.makedirs('models', exist_ok=True)
    encoder.save('models/pretrained_encoder.h5')
    print("Encoder saved to models/pretrained_encoder.h5")
